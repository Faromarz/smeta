...
class Controller_Auth extends Controller_Core{
	...
	public function action_cab() // просмотр профиля или авторизация через соц.сеть
	{
...
//                }
		...
		if (!$ulogin->mode()) // если ранее юлогин не вызывался
		{
			...
			try // пробуем
			{
				...
				if ($user['photo']!='' && Auth::instance()->get_user()->photo ==NULL) // если из соц.сети можно достать фото пользователя
				{
					$fp = fopen(DOCROOT.'media/img/avatars/'.Auth::instance()->get_user()->id.'.jpg', 'w');  // кладем в папку /uploads/users (не забудьте про права на запись!)
				...
				if ($this->session->get('redirectAfterLogin')!='') // если пользователь хотел куда-то перейти
				{ 
					$redirect = $this->session->get('redirectAfterLogin'); 
					$this->session->delete('redirectAfterLogin'); // удаляем запись об этом
		...
		{
			HTTP::redirect('/auth/registration'); // редиректим на страницу со входом/регистрацией
		}

		$data['ulogin'] = $ulogin->render(); // стартуем сессии
		$data['photo'] = '/media/img/avatars/nophoto.jpg';
		if (Auth::instance()->get_user()->photo==1) $data['photo'] = "/media/img/avatars/".Auth::instance()->get_user()->id.".jpg"; // выводим фото
		...
		$this->template->v_body = View::factory('account',$data);
	...
	{
                                        $this->template->v_body->v_page = View::factory('auth/v_auth');
		...
		if (HTTP_Request::POST == $this->request->method())  // если переданы POST данные
		{
                 ...
                 echo   $this->auth->hash($this->request->post('password'));
...
//		$data['ulogin'] = $ulogin->render(); // рисуем значки соц.сетей
		...
		$data['username'] = array_key_exists('username', $this->request->post()) ? htmlspecialchars($this->request->post('username')) : ''; // вставляем данные в формы, если они были введены
                ...
                $this->template->v_body->v_page->set($data);
		...
		{			
			try {
				...
				$object
					...
					->label('password', __('Пароль'))
					->label('confirm_password', __('Повторіть пароль'))
					...
					->rule('confirm_password', 'matches', array(':value', 'password', ':field'))
                                                                                            ;
...
//		$data['ulogin'] = $ulogin->render();  // рисуем значки соц.сетей
	...
	public function action_forgot() // сброс пароля
	{
		...
		$data['email'] = array_key_exists('email', $this->request->post()) ? htmlspecialchars($this->request->post('email')) : '';
		  $this->template =  View::factory('index/page/v_auth_forgot', $data);
