...
class Controller_Admin_Articlesblock extends Controller_Admin_Index
{
	/**
	...
	public function before() {
		parent::before();
                 $this->template->scripts =array( '/resources/libs/underscore.js');
            $this->template->scripts[] = '/resources/libs/eventable.js';
             $this->template->scripts[] = '/resources/libs/sir-trevor-js-master/sir-trevor.js';
            
            $this->template->styles[] = '/resources/libs/sir-trevor-js-master/sir-trevor.css';
...
//		);
	...
	{
		$object = ORM::factory('article')->where('block', '=', 1);
		
		$pagination = $object->get_pagination();
		
		$this->template->v_body->v_page  = View::factory('admin/blocks/articlesblock/index');
...
//		$this->template->v_body->v_page ->objects	 = $object->get_objects($pagination);
	...
	{
		$this->template->v_body->v_page  =  View::factory('admin/blocks/articlesblock/edit');
		...
		if ($_POST)
		{
			try
			{
				...
				HTTP::redirect("/admin/articlesblock/");
			}
		...
		$this->template->v_body->v_page ->title = 'Добавить статтю';
	...
	public function action_delimg()
	{
		...
		if ( $object->loaded() ) {
			...
			$object->img = NULL;
                        $object->save();
			HTTP::redirect('/admin/articlesblock/edit/'.$this->request->param('param'));
