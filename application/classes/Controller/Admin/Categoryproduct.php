...
class Controller_Admin_Categoryproduct extends Controller_Admin_Index {
...
//		);
    ...
    public function action_index() {
        $this->template->v_body->v_page = View::factory('admin/blocks/categoryproduct/index')
                          ->bind('errors', $errors)
                          ->bind('object', $object);
        ...
        if (isset($_POST['add']) && !empty($_POST['name'])) {
            $cat = ORM::factory('Categoryproduct');
            $cat->name = $_POST['name'];
            if ($_POST['id'] == 0) {
                if (ORM::factory('Categoryproduct')->where('name', '=', $_POST['name'])->where('parent_id', '=', 0)->find()->loaded()) {
                    $errors = array('Категория "' . $_POST['name'] . '" уже добавлена');
                } else {
                    $cat->make_root();
                }
            } else {
                $cld = ORM::factory('Categoryproduct', (int) $_POST['id'])->children();
                if (count($cld)> 0 && ORM::factory('Categoryproduct')->where('name', '=', $_POST['name'])->where('parent_id', '=', $cld[0]->parent_id)->find()->loaded()) {
            ...
            $cat = ORM::factory('Categoryproduct', $_POST['id']);
            $est = ORM::factory('Categoryproduct')->where('name', '=', $_POST['name1'])->where('parent_id', '=', $cat->parent_id)->find();
            if ($est->loaded() && $est->id != $_POST['id']) {
        ...
        $object = ORM::factory('Categoryproduct')->fulltree();
