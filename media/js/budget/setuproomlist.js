...
function SetupRoomList($parent)
{
    ...
    _this.updateMaterials = function()
    {
        ...
        var _room;
        var content = '<div class="budget_basic_options_header">'+                   
            '<dl>'+
                '<dd>0,00</dd>'+
                '<dt>Общая сумма:</dt>'+
            '</dl>'+
    ...
    _this.initRooms = function() {
        ...
        for (room in rooms) {
            ...
            _params = {
                ...
                enable: rooms[room].enable
            };
            if (in_array(room, [0, 1, 2, 3, 4])) {
                if(rooms[room].enable){
                    _this.roomCount++;                    
                }
                _params.window = [];
                _params.balcon = false;
                if (_params.id === 1) {
                    _params.door = [];
                } else {
                    _params.show = rooms[room].show;
                }
            } else if (room === 5) {
                _params.window = [];
                _params.balcon = false;
    ...
    _this.showRoomAdd = function() {
        ...
        if (roomCountMax > room) {
            if (room >= 1) {
                ...
                $('#add_window').parent().children('.smeta_window').eq(0).clone(true).insertBefore($('#add_window').parent().children('.smeta_window[data-window]').last()).data('window', 1).data('id-room', _parent.rooms.room[room].getId);
            }
