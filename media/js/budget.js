...
$(document).ready(function() {
    ...
    $("#change_budget").on("click", function() {
        $("#budget_dop_options").slideDown(350);
        $("#budget_dop_options-hide").fadeIn(400, function() {
            mail_top = $("#mail").offset().top;
        });
    });
    $("#budget_dop_options-hide").on("click", function() {
        $("#budget_dop_options").slideUp(350);
        $("#budget_dop_options-hide").fadeOut(400, function() {
            mail_top = $("#mail").offset().top;
        });
    });

    $(".budget_basic_options").on("click", ".budget_basic_options_job", on_input).on("blur", "input", off_input).on("keyup", "input", function(event) {
        if (event.keyCode === 13) {
            off_input.call(this);
        }
    });
    ...
    firm.hover(function() {
        $(this).children(".firm_conteyner").css("margin-top", "-170px");
    }, function() {
        $(this).children(".firm_conteyner").css("margin-top", "0");
    }).on("click", function() {
        firm.removeClass('firm_selected');
        $(this).addClass("firm_selected");
    });
    ...
    slide_menu.slideToggle(400, function() {
        ...
        if (slide_menu.is(':visible')) {
            ...
            interval = setInterval(function() {
                switch (count) {
                    case 1:
                        $(".budget_repair_estimate-slide_menu-slide1").fadeOut(400, function() {
                            $(".budget_repair_estimate-slide_menu-slide2").fadeIn();
                        });
                        count = 2;
                        break;
                    case 2:
                        $(".budget_repair_estimate-slide_menu-slide2").fadeOut(400, function() {
                            $(".budget_repair_estimate-slide_menu-slide3").fadeIn();
                        });
                        count = 3;
                        break;
                    case 3:
                        $(".budget_repair_estimate-slide_menu-slide3").fadeOut(400, function() {
                            $(".budget_repair_estimate-slide_menu-slide1").fadeIn();
                        });
                        count = 1;
                        break;
                }
