$(document).ready(function () {
    ...
    $("#lk-company-logo-for_logo")
        .on("mouseenter", "#lk_company-logo", function() { 
            $(this).parent().children(".lk_company-logo-load").fadeIn();
        })
        .on("mouseleave", ".lk_company-logo-load", function() {
            $(this).fadeOut();
        });
    $("#lk-content-personal_data")
        .on("keypress", "textarea", function () {
            $("#lk_company-content-save_change").css("background", "#f7540f");
        })
...
//            })
    ...
    $("#lk_company-examples_works").on("mouseenter", ".lk_company-examples_works-block", function() {
        ...
        $(this).children(".lk_company-examples_works-block-change").fadeOut(); }).on("click", ".lk_company-examples_works-block", change_photo  );
    $("#lk_company-close_pop_up").on("click", close_change_photo);

    //сворот/разворот в прайс-листе
    $("#lk_company-price").on("click", ".lk_company-price-block-header", function(){
        $(this).toggleClass('lk_company-price-block-header-up lk_company-price-block-header-down').parent().children('ul').slideToggle();
        var per = $(this).children('h2').text();
        if (per == 'Демонтажные работы') {
            $("#dismantling").parent('.lk_company-price-for_save_change').fadeToggle();
        } else {
            $("#installation").parent('.lk_company-price-for_save_change').fadeToggle();
        }
    } );
